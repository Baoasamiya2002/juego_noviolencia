################################################################################
##
## Truncated Text for Ren'Py by Feniks (feniksdev.itch.io / feniksdev.com) v1.2
##
################################################################################
...
##
## substitute - defaults to True, which means variables and other text
## formatting will be performed on the provided text. This should always be
## True for proper truncation, but can be set to False in the event that the
## text you are passing in has already had substitution performed.
...
python early:
    ...
    class TruncateText(Button, Window):
        ...
        substitute : bool
            If True, the default, variables and other text formatting will be
            performed on the provided text. This should always be True for
            proper truncation, but can be set to False in the event that the
            text you are passing in has already had substitution performed.
        has_tooltip : bool
        ...
        def __init__(self, text, *args, **kwargs):
            ...
            self.substitute = kwargs.pop("substitute", True)
            ...
            if not self.truncated_text:
                ...
                if txt_width <= aw and txt_height <= ah:
                    ...
                    if not successful_shrink and self.truncate_after_shrink:
                        ...
                        splits = ttext.valid_split_points(the_text, self.separators)
                        for i in reversed(splits):
                            txt = Text(ttext.check_text_tags(the_text[:i]) + self.suffix,
                                **new_text_properties)
                            txt_width, txt_height = txt.render(aw, ah, st,
                                at).get_size()
                            if self.has_tooltip == "rest":
                                self.remaining_text = ttext.get_opening_tags(the_text,
                                    the_text[i:])
                            if txt_width <= aw and txt_height <= ah:
                                self.truncated_text = txt
...
init python in ttext:
    from collections import Counter
    import re
    from store import TruncateText
